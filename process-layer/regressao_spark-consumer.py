...
class RDDProcessor:
    ...
    def pre_process(self, rdd):
        if not rdd.isEmpty():
            ...
            predictions = []
            predictions_rf = modelorf.transform(obj__final)
            output_rf = predictions_rf.select("prediction").collect()
            ...
            #X_super = np.array(list(map(mont_feat, prediction_rf.select("probability").collect(),
                                       #prediction_gb.select("probability").collect())))
            #predictions = supermodelo.predict(X_super)

            for f, t in zip(prediction_false, prediction_true):
                if t > f:
            ...
            for ln1, ln2, ln3, ln4, ln5, ln6 in zip(predictions, s_classe, X, prediction_true, prediction_false, output_rf):
                ...
                if len(texto) > 50:
                    ...
                    supermodelo.partial_fit(X_inc, y_inc)
                    os.remove('incremental.txt')
                arq.close()
                '''


rddProcessor = RDDProcessor()
flows.foreachRDD(lambda rdd: rddProcessor.pre_process(rdd))
